...
> 生产环境里踩出来的，带完整排查链路与源码定位。

| 文档 | 一句话定位 |
| --- | --- |
| [@LoadBalanced 把服务名变成 IP 的全过程（案例）](./LoadBalanced把服务名变成IP的全过程：拦截器、注册表与跨语言顶替（案例）.md) | 拦截器链路到类名级 / **注册表是表不是契约**（一个 Go 服务顶替 Spring Boot）/ 实例三种来路与元数据反推 / 顶替服务的七项契约清单 |
...
| `@LoadBalanced` 怎么让 RestTemplate 认识服务名？ | [@LoadBalanced 全过程（案例）](./LoadBalanced把服务名变成IP的全过程：拦截器、注册表与跨语言顶替（案例）.md) |
| 服务名为什么不能带下划线？ | [@LoadBalanced 全过程（案例）](./LoadBalanced把服务名变成IP的全过程：拦截器、注册表与跨语言顶替（案例）.md) |
| 非 Java 服务能注册进 Nacos 被调用吗？ | [@LoadBalanced 全过程（案例）](./LoadBalanced把服务名变成IP的全过程：拦截器、注册表与跨语言顶替（案例）.md) |
| Nacos 实例的 `healthy` / `enabled` / `ephemeral` 有什么区别？ | [@LoadBalanced 全过程（案例）](./LoadBalanced把服务名变成IP的全过程：拦截器、注册表与跨语言顶替（案例）.md) |
...
15. **Java 应用替外部服务「代注册」**：`registerInstance()` 建的是临时实例、心跳由代注册方发 → 那个 Java 应用一重启，被它代注册的 Python/GPU 服务 30 秒内集体从注册表消失（而它们活得好好的）。→ [@LoadBalanced 全过程（案例）](./LoadBalanced把服务名变成IP的全过程：拦截器、注册表与跨语言顶替（案例）.md)
16. **查 Nacos 不传 `namespaceId` 默认查 `public`**：实例在别的 namespace 里，API 报 `no ips found`，看着像实例掉了——报错**不会提示你 namespace 可能选错**；`/instance/list` 还会过滤掉 `enabled=false` 的实例（控制台却显示）。→ [@LoadBalanced 全过程（案例）](./LoadBalanced把服务名变成IP的全过程：拦截器、注册表与跨语言顶替（案例）.md)
